...
void Bridge_send_Serial1_Measure(struct Bridge_Instance *_instance, long m);
...
long l;
		...
		switch(msgID){
			case 1://Measure
				if(_instance->Bridge_MsgSize__var == 8) {
					l = ((_instance->Bridge_Buffer__var[4] << 24));
					l += ((_instance->Bridge_Buffer__var[5] << 16));
					l += ((_instance->Bridge_Buffer__var[6] << 8));
					l += ((_instance->Bridge_Buffer__var[7]) & 0xFF);
					printf("Buffer: 4:%i", _instance->Bridge_Buffer__var[4]);
					printf(" 5:%i", _instance->Bridge_Buffer__var[5]);
					printf(" 6:%i", _instance->Bridge_Buffer__var[6]);
					printf(" 7:%i", _instance->Bridge_Buffer__var[7]);
					printf(" -> l:%i where", l);
					printf(" -> shifted6:%i \n", ((_instance->Bridge_Buffer__var[6] << 8)));
					float * f = (float *) &l;
					
Bridge_send_Serial1_Measure(_instance, l);
...
void (*Bridge_send_Serial1_Measure_listener)(struct Bridge_Instance*, long)= 0x0;
